...
void printInstruction(word p[], word pc)
{
  ...
  case SETCDR:
    ...
    break;
  case PUSHLAB:
    printf("PUSHLAB " WORD_FMT, p[pc + 1]);
    break;
  case HEAPSTI:
    printf("HEAPSTI " WORD_FMT, p[pc + 1]);
    break;
  case HEAPLDI:
    printf("HEAPLDI " WORD_FMT, p[pc + 1]);
    break;
  case ACLOS:
    printf("ACLOS " WORD_FMT, p[pc + 1]);
    break;
  case CLOSCALL:
    printf("CLOSCALL " WORD_FMT, p[pc + 1]);
    break;
  case TCLOSCALL:
    printf("TCLOSCALL " WORD_FMT, p[pc + 1]);
    break;
  case THROW:
    printf("THROW");
    break;
  case PUSHHDLR:
    printf("PUSHHDLR " WORD_FMT, p[pc + 1]);
    break;
  case POPHDLR:
    printf("POPHDLR");
    break;
  default:
    printf("<unknown> " WORD_FMT, p[pc]);
    break;
  }
}
...
void printL(word i)
{
  ...
  else
  {
    ...
    do
    {
      ...
      word tl = consPtr[2];
      if (IsInt(hd))
        printf(WORD_FMT, Untag(hd));
      else
      {
        // int *vPtr = (int *)hd;
        if (hd == NILVALUE || (BlockTag(*((word *)hd)) == CONSTAG))
          printL(hd);
        else
          printf("Unexpected hd=" WORD_FMT "\n", hd);
      }
      if (tl != NILVALUE)
      {
        consPtr = (word *)tl;
